        ...
        for key, value in aliments_by_categorie.items():
            for element_dict in value:
                if "stores" in element_dict.keys():
                    list_all.append(Aliments(url=element_dict["url"], id_off=element_dict["id"], categorie=key, stores=element_dict["stores"], nutrition_grades=element_dict["nutrition_grades"], product_name=element_dict["product_name"]))
                else:
